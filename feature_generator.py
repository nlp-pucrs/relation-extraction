...
def generate_features(name):
	...
	for idx, line in enumerate(file):
		...
		if(sentence.find('en1') > sentence.find('en2')):
			possible_descriptor = sentence[sentence.find('en2')+3:sentence.find('en1')].split(' ')
		else:
			possible_descriptor = sentence[sentence.find('en1')+3:sentence.find('en2')].split(' ')
		curr_rel = 0
		...
		if train_set:
			...
			for idx, t in enumerate(analyzer):
				new_sentence.append(t.lexeme)

			sentence = ' '.join(new_sentence)
			for r in rel:
				if r != '':
					count_descriptor = 0
					for pd in possible_descriptor:
						if pd == r:
							count_descriptor +=  1
					if(sentence.find('en1') > sentence.find('en2')):
						end_pass = sentence.find('en1')
					else:
						end_pass = sentence.find('en2')
					descript_list = []
					for x in range(count_descriptor):
						descript_list.append(sentence[:end_pass].rfind(r))
						end_pass = sentence[:end_pass].rfind(r)
					descript_list = descript_list[::-1]
					minDist = None
					if len(descript_list) == 0:
						index_change = 0
					else:
						for dl_index in descript_list:
							index_this = len(sentence[:dl_index].split(' '))
							dist = abs(int(rel_num[curr_rel]) - index_this)
							if minDist == None:
								minDist = dist
								index_change = 0
							elif minDist > dist:
